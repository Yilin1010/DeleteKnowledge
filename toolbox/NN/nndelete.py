# DW dot Wdel,just for last layer
import numpy as np
...
def nndelete2(nn, xi, train_x, train_y,samelabel, opts,figureNo,ifepochs):             
...
# DW dot d[]
    numlabel=deleted_y.shape[1]
    Wdel = np.identity(numlabel) 
    Wdel[samelabel][samelabel] = -1 # = deleted sample label

    for i in range (0,n-1):
        if (i+1) == (n-1):
            term = d[i+1]
            nn.DW[i] = np.dot(term.T,nn.A[i])/d[i+1].shape[0]
            nn.DW[i] = np.dot(nn.DW[i].T,Wdel).T # last layer
            nn = nnapplygrads(nn)
        else:
            term = d[i+1][:,1:]            
            nn.DW[i] = np.dot(term.T,nn.A[i])/d[i+1].shape[0]

        # nn = nnapplygrads(nn) # -i
